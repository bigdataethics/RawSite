...
Coffee shops are generally a popular place to hangout. Starbucks, Colectivo, Anodyne, Stone Creek, etc. are some of the popular coffee shops in Milwaukee. It is not just the rejuvenating coffee that attracts customers but the shop ambience as well. It is common to find people reading books, working on their laptops, writing, being in deep thought, carrying on conversations, or just relaxing. But sometimes getting a cup of coffee can take a long wait. Coffee shops try to minimize the wait time so that customers are satisfied with the service.

In this tutorial we will simulate a simple coffee shop. The items on order are: Regular Coffee (10-15), Latte (30-45), Mocha (30-45), Cold Brew (10-20), Frappe (50-70), and Espresso (20-35). The numbers in brackets indicate how many seconds it takes to prepare the item. 

When a customer arrives they go to the cashier to place the order and make the payment. The cashier accepts payments through cash or cards. It generally takes 15 to 30 seconds to service a payment by cash and 10 to 20 seconds to service a payment by card. Once the payment is made the item is prepared by the barista. When the item is ready the customer picks up the order.

The purpose of this simulation is to see how the number of cashiers and baristas affect the wait times of customers. We will use Python with some relevant libraries to model this simulation.
...
- Generate a random number to indicate payment by cash or card: `payment_type = random.randint(1,2)`
...
We are simulating for 20 customers and a coffee shop with 2 cashiers and 3 baristas. `menu` contains the items that may be ordered and their preparation time ranges.  `payment_options` contains the options for making the payment and the time range it takes to process the payment.
...
We then define the environment and resources. And we start the process of generating customers. The environment is run for 300 seconds.
